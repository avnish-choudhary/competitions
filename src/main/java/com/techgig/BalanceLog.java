...
public class BalanceLog {
    public double balancePoint(Log log) {
        ...
        double totalWeight = log.weightUpto(log.length());

        double halfWeight = totalWeight / 2;

        double x = balance(halfWeight, 0.001,2, log);
        while (log.weightUpto(x) < halfWeight) {
            x += 0.001;
    ...
    double balance(double target, double x, int factor, Log log) {
        if (log.weightUpto(x) < target) {
            factor++;
            x *= factor;
            balance(target, x,factor, log);
        } else if (log.weightUpto(x) == target) {
            return x;
        } else if(factor>2)
        {
            factor--;
